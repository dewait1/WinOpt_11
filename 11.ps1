...
# Встановлюємо Visual Studio Code
choco install vscode -y --params "/NoDesktopIcon"

# Встановлюємо GIT
choco install git.install

# Встановлюємо Spotify
...
$downloadPath = "$env:USERPROFILE\AppData\Local\Mova\Mova.exe"


# Створення директорії для збереження файлу, якщо її ще не існує
$destinationFolder = "$env:USERPROFILE\AppData\Local\Mova"
...
Write-Host  -ForegroundColor DarkMagenta "Запускаємо O&O Shutup"
# Попереду одне із моїх дивних рішень. Щоб не завантажувати файл конфігурації телеметрії для застосунка O&O Shutup, я просто його зчитаю прямо з цього файла та запишу у окремий файл. 
# Таким чином ми викличемо вимкнення телеметрії завантаживши лише O&O Shutup. І файл і O&O Shutup видаляться після того, як робота буде виконана.
$configText = @"
...
# Цю частину лишаю закоментованою (значить ця частина скрипта не працюватиме), бо вона ВИмикає відслідковування локації ПК. 
# Дуже потрібна штука в мапах тощо. Також закоментував скрипт, що вимикає звітування про помилки Windows. Код залишаю, якщо комусь треба це вимикати
<#
...
# Ввімкнення P2P Update тільки всередині локальної мережі
...
# За допомогою повідомлень через WAP_Pushing можна отримати фішінгові повідомлення, або повідомлення, що завантажують шкідливий код. 
# Проте їх вимкнення не дасть вам змогу отримувати повідомлення про важливі оновлення тощо. Нехай працює собі, якщо ви не людина, що клікає на всі повідомлення про спортлото.
<#
...
# Remote Assistance у Windows є потужним інструментом для отримання або надання допомоги на відстані. Це особливо корисно в корпоративному середовищі, де технічна підтримка може швидко і ефективно вирішувати проблеми користувачів. 
# Для домашнього користування, користувачі надають перевагу стороннім застосункам, таким, як AnyDesk. Тож можна видалити.
Write-Host  -ForegroundColor DarkMagenta "Вимикаємо Remote Assistance..."
Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Remote Assistance" -Name "fAllowToGetHelp" -Type DWord -Value 0

# Гібернація — це енергозберігаючий стан, який дозволяє зберігати стан комп'ютера на жорсткому диску, а потім повністю вимикати систему. 
# На стаціонарному ПК не бачу в цьому ніякого сенсу. Для ноутбуків же краще видалити (або занотувати) рядки, що вимикають гібернацію, бо там ця функція стане у пригоді.
...
# Увімкнення Num Lock при запуску — це зручна конфігурація для багатьох користувачів, яка дозволяє уникнути додаткових дій та забезпечує комфортне використання числової клавіатури одразу після завантаження системи. 
# Це особливо корисно для тих, хто часто працює з числовими даними або паролями, що містять цифри.
...
<#
# Зміна домашньої сторінки при відкритті провідника
...
# Зазвичай параметр IRPStackSize встановлюється для збільшення стеку IRP в системі, що дозволяє краще управляти мережевими операціями і зменшити можливість виникнення помилок, пов'язаних з перевищенням розміру стеку. 
# Це особливо важливо для великих мережевих середовищ або в умовах високого навантаження на сервер.
...
# Autologger - файл у який пишеться дуже багато телеметрії Microsoft і потім відправляється на опрацювання. 
# Якщо ви також не хочете, щоб про вас збирали та відправляли якісь дані, то вимикаємо це все діло, та забороняємо системі створювати нові файли в тій теці на відправку.
...
$services = @(
    ...
    #"WerSvc"                                      # ідповідає за звітування про помилки. Ось це, я вважаю, можна і залишити. Про помилки в системі Microsoft варто знати.
...
$taskDescription = "Створення щотижневої точки відновлення у неділю об 12:00"
$triggerTime = "12:00"
...
# Створити тригер (щонеділі об 12:00)
